...
shinyServer(function(input, output,session) {
  ...
  observe({
    if(input$editAnalysis & (input$AnalysisOrder=="")) updateTextInput(session,"AnalysisOrder",value=myfittext())
...
output$dataHelp<-renderPrint({
   if(length(grep(".csv",input$Example))==0) {
   ...
   else cat("No help about ",input$Example)
...
#           cat("\n\n")
         ...
         if(input$method=="matrixpls") {
           ...
           if((input$group!="") & input$showMeaInv) {
             cat("\nmeasurementInvariance(input$equation,data=df(),group='",input$group,"'\n")
